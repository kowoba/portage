...
# $Header: /var/cvsroot/gentoo-x86/sys-libs/glibc/glibc-2.5-r4.ebuild,v 1.15 2009/12/10 01:31:26 vapier Exp $
...
toolchain-glibc_src_unpack() {
	...
	if use m68k ; then
		EPATCH_OPTS="-p1 -d${S}" epatch "${FILESDIR}"/2.5/glibc-2.5-debian.patch.bz2
		epatch "${FILESDIR}"/2.5/glibc-2.5-debpatch.patch
		epatch "${FILESDIR}"/2.5/glibc-2.5-binutils220.patch
		ln -s glibc-ports-2.5 ${S}/ports
		ln -s glibc-libidn-2.5 ${S}/libidn
		rm ${S}/manual/Makefile
		cat ${S}/debian/patches/series | grep -v -e '^\#' -e '^$' | while read patch plevel ; do
			EPATCH_OPTS="${plevel} -d${S}" epatch ${S}/debian/patches/$patch
		done
	fi
